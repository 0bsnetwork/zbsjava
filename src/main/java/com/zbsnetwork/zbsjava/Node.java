...
import com.zbsnetwork.zbsjava.matcher.CancelOrder;
import com.zbsnetwork.zbsjava.matcher.DeleteOrder;
import com.zbsnetwork.zbsjava.matcher.Order;
import com.zbsnetwork.zbsjava.transactions.LeaseTransaction;
...
public class Node {
    ...
    public Node(String uri, char chainId) throws URISyntaxException {
        ...
        this.ZbsJsonMapper = new ZbsJsonMapper((byte) chainId);
        this.client = createDefaultClient();
    }
    ...
    public Transaction getTransaction(String txId) throws IOException {
        return ZbsJsonMapper.convertValue(send("/transactions/info/" + txId), Transaction.class);
    }

    public Map<String, Object> getTransactionData(String txId) throws IOException {
        return ZbsJsonMapper.convertValue(send("/transactions/info/" + txId), TX_INFO);
    }
    ...
    public Block getBlock(int height) throws IOException {
        return ZbsJsonMapper.convertValue(send("/blocks/at/" + height), Block.class);
    }
    ...
    public Block getBlock(String signature) throws IOException {
        return ZbsJsonMapper.convertValue(send("/blocks/signature/" + signature), Block.class);
    }
    ...
    public Order createOrder(PrivateKeyAccount account, String matcherKey, AssetPair assetPair, Order.Type orderType,
        ...
        return ZbsJsonMapper.treeToValue(tree.get("message"), Order.class);
    }
    ...
    private HttpUriRequest request(ApiJson obj) throws JsonProcessingException {
        ...
        request.setEntity(new StringEntity(ZbsJsonMapper.writeValueAsString(obj), ContentType.APPLICATION_JSON));
        return request;
    }
    ...
    private <T> T parse(HttpResponse r, TypeReference<T> ref) throws IOException {
        return ZbsJsonMapper.readValue(r.getEntity().getContent(), ref);
    }

    private JsonNode parse(HttpResponse r, String... keys) throws IOException {
        JsonNode tree = ZbsJsonMapper.readTree(r.getEntity().getContent());
        for (String key : keys) {
            tree = tree.get(key);
        }
        return tree;
