...
import static com.zbsnetwork.zbsjava.DataEntry.*;
...
public class NodeTest {
    ...
    private static final AssetPair MARKET = new AssetPair(Asset.ZBS, WBTC);
    ...
    public void testGetters() throws IOException {
        ...
        long zbsBalance = node.getBalance(address);
        assertTrue(zbsBalance >= 0);
        assertEquals(zbsBalance, node.getBalance(address, null));
        assertEquals(zbsBalance, node.getBalance(address, ""));
        assertEquals(zbsBalance, node.getBalance(address, Asset.ZBS));
    }
    ...
    public void testTransfer() throws IOException {
        ...
        txId = node.send(new TransferTransactionV1(bob, alice.getAddress(), AMOUNT, Asset.ZBS, FEE * 5, null, new ByteString("Thanks again".getBytes()), now + 3));
        assertNotNull(txId);
    }
    ...
    public void testMassTransfer() throws IOException {
        ...
        String txId = node.massTransfer(alice, Asset.ZBS, transfers, FEE * 2, "sharedrop");
        assertNotNull(txId);
    ...
    public void testDataTransaction() throws IOException {
        ...
        BinaryEntry bin = new BinaryEntry("This data was proudly published using zbsjava (https://github.com/0bsnetwork/zbsjava)",
                new ByteString("zbsjavarocks"));
    ...
    public void testSendTransaction() throws IOException {
        ...
        TransferTransactionV2 tx1 = Transactions.makeTransferTx(alice, bob.getAddress(), AMOUNT, Asset.ZBS, FEE * 5, Asset.ZBS, "To Bob");
        String id1 = node.send(tx1);
        assertNotNull(id1);
        assertEquals(id1, tx1.getId().getBase58String());

        MassTransferTransaction tx2 = Transactions.makeMassTransferTx(bob, Asset.ZBS,
    ...
    public void testMatcher() throws IOException {
        ...
        assertEquals(Asset.ZBS, order.getAssetPair().getAmountAsset());
